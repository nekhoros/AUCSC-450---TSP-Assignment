...
int selectNextCity(int current, vector<int>& visited, vector<double>& pheromone, const vector<pair<int, int>>& points) {
    ...
    // First loop: calculate probabilities
    ...
    // Second loop: normalize probabilities
...
int main() {
    ...
    #pragma omp parallel
    {
        for (int iter = 0; iter < N_ITER; ++iter) {
            ...
            #pragma omp for schedule(dynamic) reduction(min:bestLength)
            ...
            // Synchronize before pheromone updates
            #pragma omp barrier
